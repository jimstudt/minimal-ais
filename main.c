...
{
    fprintf(stderr,
            "usage: %s -s serial [-b baud] [-v] -d host:port [-d host:port ...]\n"
            "\n"
            "  -s serial     serial device, for example /dev/ttyUSB0\n"
            "  -b baud       serial baud rate, default 38400\n"
            "  -d host:port  UDP destination; repeat for more destinations\n"
...
static int add_dest(const char *spec, struct dest *dests, size_t *ndests)
{
    ...
    if (parse_dest_spec(spec, host, sizeof(host), port, sizeof(port)) < 0) {
        fprintf(stderr, "invalid destination '%s'; use host:port or [ipv6]:port\n", spec);
        return -1;
    }
...
static void forward_line(const char *line, const struct dest *dests, size_t ndests)
{
...
int main(int argc, char **argv)
{
    ...
    for (;;) {
        ...
        forward_line(line, dests, ndests);
    }
